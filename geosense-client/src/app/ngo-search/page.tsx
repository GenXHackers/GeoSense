...
export default function NgoSearch() {
  ...
  useEffect(() => {
    const fetchStates = async () => setStates(await data.getStates());
    const fetchSectors = async () => setSectors(await data.getSectors());
    fetchStates();
    fetchSectors();
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchDistricts = async () =>
      setDistricts(await data.getDistricts(selectedState));
    fetchDistricts();
  }, [selectedState]);
  ...
  const getSectorFromValue = (value: string) => {
    return sectors?.filter((v) => value === v[0])![0];
  };
  ...
  const handleSearchButtonClicked = async () => {
    ...
    await data.search(
      (obj) => {
        if (obj["type"] === "name") {
          const data = { ...obj.data, hasLoaded: false };
          setResults((res) => (res ? [...res, data] : [data]));
        } else if (obj["type"] === "end") {
          return;
        } else {
          const data = { ...obj.data, hasLoaded: true };
          setResults((res) => [
            ...(res?.filter(
              (v) => v["ngo_name_title"] !== data["ngo_name_title"]
            ) ?? []),
            data,
          ]);
        }
  ...
  return (
    <div className="p-4 flex flex-col space-y-2 pb-24 select-none">
        ...
        {selectedSectors && selectedSectors.length > 0 ? (
          <ul className="flex flex-wrap gap-2 p-2">
            {selectedSectors.map((v, i) => {
              const sect = getSectorFromValue(v)!;
              return (
                <li
                ...
                >
                  <p>{sect[1]}</p>
                  <button onClick={() => handleSectorChipClicked(sect[0])}>
